$(function () {
    // select 元素
    const $selectorProvince = $("#province");
    const $selectorCity = $("#city");
    const $selectorDistrict = $("#district");

    // 地区的默认值，通过select的default-data获取
    let defaultProvince = $selectorProvince.attr('data-default');
    let defaultCity = $selectorCity.attr('data-default');
    const defaultDistrict = $selectorDistrict.attr('data-default');
    ...
    function initSelector(selectObj, data) {
        ...
        let str = "";
        const selected = selectObj.attr('data-default');
